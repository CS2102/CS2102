...
public class Examples {
	...
	public boolean isAVL(IBST tree) {
		if (tree.height() == 0) {return true;}
			...
			boolean currBalanced = Math.abs(tree.getLeft().height() - tree.getRight().height()) <= 1;
			return currBalanced &&
					isAVL(tree.getLeft()) &&
					isAVL(tree.getRight());
		}
	...
	boolean testAVL12(Tester t){
		A = new EmptyAVLTree();
		return t.checkExpect(isAVL((AVLTree)A.addElem(6).addElem(9).addElem(7)
				.addElem(20).addElem(1)), true);
	}
	
	boolean testAVL13(Tester t) {
		DataBST D = new DataBST(4, (IBST)new MtBST().addElem(5).addElem(4), new MtBST());
